 ...
 * Java应用运行时的初始线程的上下文类加载器是系统类的加载器。在线程中运行的代码可以通过该类加载器来加载类和资源。
 *
 * 线程上下文类加载器的重要性：
 *
 ...
 * 这就改变了父ClassLoader不能使用子ClassLoader或是其他没有直接父子关系的ClassLoader加载的类的情况，即改变了双亲委托
 * 模型。
 *
 ...
 * 来源的jar包，这样传统的双亲委托模型就无法满足SPI的需求。而通过给当前线程设置上下文类加载器，就可以由设置的上下文类加载器
 * 来实现对于接口实现类的加载。
 */
